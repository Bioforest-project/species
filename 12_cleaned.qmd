...
wfo %>% 
  ...
  summarise(N = n()) %>% 
  mutate(fuzzy = ifelse(fuzzy, "fuzzy", "exact"),
...
wfo %>% 
  ...
  group_by(fuzzy_dist) %>% 
  summarise(N = n()) %>% 
  kable(caption = "Number of misspelled species by the number of misspelled letters.")
```
...
wfo_clean <- bind_rows(wfo_exact, wfo_mult) %>% 
  select(spec.name, family, genus, specificEpithet,
         scientificName) %>% 
  rename(scientificname = spec.name, 
         family_wfo = family, 
         genus_wfo = genus,
...
data.frame(level = c("family", "genus", "species"),
           N = c(nrow(fam_cor), nrow(gen_cor), nrow(sp_cor))) %>% 
  kable(caption = "Number of synonymy, misspelling, and update corrections per taxonomic level")
```
