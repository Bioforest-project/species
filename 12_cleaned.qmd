...
Then we started by cleaning raw species data content. This corresponds to **version 3**.
...
gen_wrong <- c(
  ...
  "Unident", "Unknow", "unknown", "Unknown"
)
sp_wrong <- c(
...
corrected <- read_tsv("data/derived_data/raw_taxo_preped_v3.tsv") %>%
  rename(
...
write_tsv(corrected, "data/derived_data/corrected_taxo_v3.tsv")
```

```{r namecortab}
#| message: false
#| warning: false
read_tsv("data/derived_data/corrected_taxo_v3.tsv") %>%
...
families <- read_tsv("data/derived_data/corrected_taxo_v3.tsv") %>%
...
taxo <- read_tsv("data/derived_data/corrected_taxo_v3.tsv") %>%
...
write_tsv(cleaned, "data/derived_data/wfo_out_v3.tsv")
unlink("data/derived_data/wfo", recursive = TRUE)
```

3,911 species matched a species from the World Flora Online (WFO) except 8 that can be cured manually:

```{r matches}
#| message: false
#| warning: false
wfo <- read_tsv("data/derived_data/wfo_out_v3.tsv")
wfo %>%
  group_by(spec.name) %>%
...
taxo_cleaned <- read_tsv("data/derived_data/corrected_taxo_v3.tsv") %>%
  ...
  left_join(wfo_clean)
write_tsv(taxo_cleaned, "outputs/taxonomy_v3.tsv")
```

```{r wfo_clean_stat}
#| message: false
clean <- read_tsv("outputs/taxonomy_v3.tsv")
