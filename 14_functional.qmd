...
try <- read_tsv("data/raw_data/try/38775.txt") %>%
  ...
  select(AccSpeciesName, TraitName, OriglName, StdValue, OrigValueStr) %>%
  mutate(TraitName = recode(TraitName,
    ...
    "Plant nitrogen(N) fixation capacity" = "n_fixation",
    "Dispersal syndrome" = "dispersal",
    "Leaf phenology type" = "phenology"
  )) %>%
  mutate(value = StdValue) %>%
  mutate(value = ifelse(TraitName %in% c("dispersal", "n_fixation"),
    OrigValueStr, value
  )) %>%
  mutate(value = ifelse(TraitName %in% c("phenology"), OriglName, value)) %>%
  rename(species = AccSpeciesName, trait = TraitName) %>%
  select(-StdValue, -OriglName, -OrigValueStr)
t <- try %>%
  mutate(obs = seq_len(n())) %>%
  pivot_wider(values_from = "value", names_from = "trait") %>%
  group_by(species) %>%
  summarise(
    phenology = paste(sort(unique(na.omit(phenology))), collapse = ", "),
    n_fixation = paste(sort(unique(tolower(na.omit(n_fixation)))),
                       collapse = ", "),
    dispersal = paste(sort(unique(na.omit(dispersal))), collapse = ", "),
    wd = mean(as.numeric(wd), na.rm = TRUE),
    n = mean(as.numeric(n), na.rm = TRUE),
    sla = mean(as.numeric(sla), na.rm = TRUE),
    la = mean(as.numeric(la), na.rm = TRUE)
  ) %>%
  filter(species %in% taxo$scientific_cleaned)
taxo_try <- left_join(taxo, t, by = c("scientific_cleaned" = "species"))
```
...
We thus inferred 910 (ah,hmax) couple of parameters.
