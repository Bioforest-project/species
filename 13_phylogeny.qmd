...
We used cleaned taxonomy to build a global phylogeny using `V.PhyloMaker`. This corresponds to **version 2**.

```{r phylo}
#| eval: false
library(V.PhyloMaker)
library(ape)
splist <- read_tsv("outputs/taxonomy_v2.tsv") %>%
...
write.tree(tree$scenario.3, "outputs/phylogeny_v2.tree")
```
...
splist <- read_tsv("outputs/taxonomy_v2.tsv") %>%
  ...
  rename_all(~ gsub("_cleaned", "", .)) %>%
  mutate(species = paste(genus, species))
phylo <- ape::read.tree("outputs/phylogeny_v2.tree")
phylo_df <- phylo4d(
  phylo,
